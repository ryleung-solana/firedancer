...
impl Step for Shmem {
    ...
    fn check(&mut self, config: &Config) -> CheckResult {
        if !Path::new(&config.shmem.path).is_dir() {
            return CheckResult::Err(CheckError::NotConfigured(format!("{} does not exist", &config.shmem.path)));
        }

        check_directory(&config.shmem.path, config.uid, config.gid, 0o40700)?;
    
        for size in ["gigantic", "huge", "normal"] {
            check_directory(&format!("{}/.{size}", &config.shmem.path), config.uid, config.gid, 0o40700)?;
        }
