/* Generate prototypes, inlines and implementations for ultra high
   ...
   algorithms, including prq, assume the heap elements will be stored
   sequentially in memory).  This API is designed for ultra tight
   coupling with pools, maps, other heaps, etc.  Likewise, a heap can be
   persisted beyond the lifetime of creating process, used concurrently
   in many common operations, used inter-process, relocated in memory,
   ...
   Typical usage:
     ...
     struct myele {
       ...
       ulong left;  // Technically "HEAP_IDX_T HEAP_LEFT;" (default is ulong left), similarly for right.  These are managed by the
       ulong right; // heap when a myele is in the heap.
   ...
   compilation unit:
     ...
     // myheap_idx returns e's index.  Assumes e in ele or is NULL.
     // Return will be in [0,ele_max) or myheap_idx_null().  Lifetime is
     // the element storage lifetime.  myheap_idx_fast is the same but
     // assumes e is not NULL.  myheap_ele returns a pointer in the
     // caller's address space to element idx.  Assumes idx is in
     // [0,ele_max) or is myheap_idx_null().  Return pointer lifetime is
     // ele's local lifetime.  myheap_ele_fast is the same but assumes
     // idx is not myheap_idx_null().  myheap_ele[_fast]_const is a
     // const correct version.

     ulong           myheap_idx           ( myele_t const * e, myele_t const * ele );
     ulong           myheap_idx_fast      ( myele_t const * e, myele_t const * ele );
     myele_t *       myheap_ele           ( ulong           i, myele_t *       ele );
     myele_t *       myheap_ele_fast      ( ulong           i, myele_t *       ele );
     myele_t const * myheap_ele_const     ( ulong           i, myele_t const * ele );
     myele_t const * myheap_ele_fast_const( ulong           i, myele_t const * ele );
     ...
     ulong           myheap_idx_peek_min      ( myheap_t const * heap, char const * q, myele_t const * ele );
     myele_t *       myheap_ele_peek_min      ( myheap_t *       heap, char const * q, myele_t *       ele );
     myele_t const * myheap_ele_peek_min_const( myheap_t const * heap, char const * q, myele_t const * ele );
     ...
     // set of queries that are covered by n).  Given these assumptions,
     // this cannot fail.
     //
     ...
     myheap_t * myheap_idx_insert( myheap_t * heap, ulong     n, myele_t * ele );
     myheap_t * myheap_ele_insert( myheap_t * heap, myele_t * n, myele_t * ele );
     ...
     // Given these assumptions, this cannot fail.
     //
     ...
     myheap_t * myheap_idx_remove_min( myheap_t * heap, myele_t * ele );
     myheap_t * myheap_ele_remove_min( myheap_t * heap, myele_t * ele );

     // myheap_verify returns 0 if the myheap is not obviously corrupt
     // or a -1 (i.e. ERR_INVAL) if it is (logs details).  heap is a
     // current local join to a myheap.

     int myheap_verify( myheap_t const * heap, myele_t const * ele );
...
HEAP_STATIC HEAP_(t) * HEAP_(idx_insert)( HEAP_(t) * heap, ulong n, HEAP_T * ele );

HEAP_STATIC HEAP_(t) * HEAP_(idx_remove_min)( HEAP_(t) * heap, HEAP_T * ele );

HEAP_STATIC FD_FN_PURE int HEAP_(verify)( HEAP_(t) const * heap, HEAP_T const * ele );
...
HEAP_(idx)( HEAP_T const * e,
            HEAP_T const * ele ) {
  return fd_ulong_if( !!e, (ulong)(e-ele), HEAP_IDX_NULL );
}

FD_FN_CONST static inline HEAP_T *
HEAP_(ele)( ulong    i,
            HEAP_T * ele ) {
  return fd_ptr_if( !HEAP_IDX_IS_NULL( i ), ele + i, NULL );
}
...
FD_FN_CONST static inline ulong          HEAP_(idx_fast)      ( HEAP_T const * e, HEAP_T const * ele ) { return (ulong)(e - ele); }
FD_FN_CONST static inline HEAP_T *       HEAP_(ele_fast)      ( ulong i,          HEAP_T *       ele ) { return ele + i; }
FD_FN_CONST static inline HEAP_T const * HEAP_(ele_fast_const)( ulong i,          HEAP_T const * ele ) { return ele + i; }
...
HEAP_(ele_insert)( HEAP_(t) * heap,
                   HEAP_T *   e,
                   HEAP_T *   ele ) {
  return HEAP_(idx_insert)( heap, (ulong)(e-ele), ele );
}

static inline HEAP_(t) *
HEAP_(ele_remove_min)( HEAP_(t) * heap,
                       HEAP_T *   ele ) {
  return HEAP_(idx_remove_min)( heap, ele );
}
...
HEAP_(idx_insert)( HEAP_(t) * heap,
  ...
  if( FD_UNLIKELY( HEAP_IDX_IS_NULL( i ) ) ) { /* Heap was empty, make n the root, opt for larger heaps */
    ele[ n ].HEAP_LEFT  = (HEAP_IDX_T)HEAP_IDX_NULL;
    ele[ n ].HEAP_RIGHT = (HEAP_IDX_T)HEAP_IDX_NULL;
  ...
  for(;;) {
    ulong l = (ulong)ele[ i ].HEAP_LEFT;
    ulong r = (ulong)ele[ i ].HEAP_RIGHT;

    /* At this point, i's ancestors are less than n.  If n is before i,
       displace i with n. */

    if( FD_UNLIKELY( HEAP_(lt)( ele+n, ele+i ) ) ) { /* Opt for larger heaps and IID rank inserts */
      ele[ n ].HEAP_LEFT  = (HEAP_IDX_T)l;
      ele[ n ].HEAP_RIGHT = (HEAP_IDX_T)r;
      *_p_child = (HEAP_IDX_T)n;
    ...
    if( FD_UNLIKELY( (is_null_left | is_null_right) ) ) { /* Opt for larger heaps */
      ele[ n ].HEAP_LEFT  = (HEAP_IDX_T)HEAP_IDX_NULL;
      ele[ n ].HEAP_RIGHT = (HEAP_IDX_T)HEAP_IDX_NULL;
      *fd_ptr_if( is_null_left, &ele[ i ].HEAP_LEFT, &ele[ i ].HEAP_RIGHT ) = (HEAP_IDX_T)n;
      break;
    }
    ...
    /* At this point, i has a left and right subheap.  We need to pick one
       ...
       work only in pure heapsort like cases), we pseudo randomly pick
       one instead. */
    ...
    _p_child = fd_ptr_if  ( go_left, &ele[ i ].HEAP_LEFT, &ele[ i ].HEAP_RIGHT );
...
HEAP_(idx_remove_min)( HEAP_(t) * heap,
  ...
  HEAP_IDX_T * _p_child = &heap->root;
  ulong        l        = (ulong)ele[ d ].HEAP_LEFT;
  ulong        r        = (ulong)ele[ d ].HEAP_RIGHT;
  ...
  for(;;) {
    ...
    int promote_left = HEAP_(lt)( ele + l, ele + r );

    ulong c     = fd_ulong_if( promote_left, l, r );
    ulong l_nxt = (ulong)ele[ c ].HEAP_LEFT;
    ulong r_nxt = (ulong)ele[ c ].HEAP_RIGHT;

    *_p_child = (HEAP_IDX_T)c;

    *fd_ptr_if( promote_left, &ele[ l ].HEAP_RIGHT, &ele[ r ].HEAP_LEFT ) = (HEAP_IDX_T)fd_ulong_if( promote_left, r, l );

    _p_child = fd_ptr_if( promote_left, &ele[ l ].HEAP_LEFT, &ele[ r ].HEAP_RIGHT );
    l        = l_nxt;
    r        = r_nxt;
  }
...
# define HEAP_TEST( c ) do { if( FD_UNLIKELY( !(c) ) ) { FD_LOG_WARNING(( "FAIL: " #c )); return -1; } } while(0)
  ...
  ulong ele_max = heap->ele_max; HEAP_TEST( ele_max<=HEAP_IDX_NULL ); /* Validate ele_max */
  ulong ele_cnt = heap->ele_cnt;
  HEAP_TEST( ele_cnt<=ele_max       ); /* Validate ele_cnt */
  if( ele_max ) HEAP_TEST( ele );                                     /* Validate ele storage */
  ...
  while( stack_cnt ) { /* While still nodes to visit */
    ...
    if( !HEAP_IDX_IS_NULL( r ) ) {
      HEAP_TEST( HEAP_(lt)( ele + i, ele + r ) ); /* Make sure heap property satisfied */
    ...
    if( !HEAP_IDX_IS_NULL( l ) ) {
      HEAP_TEST( HEAP_(lt)( ele + i, ele + l ) ); /* Make sure heap property satisfied */
