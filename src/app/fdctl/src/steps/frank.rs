...
impl Step for Frank {
    ...
    fn step(&mut self, config: &mut Config) {
        ...
        run!(bin, "fd_wksp_ctl", ["new", workspace, config.workspace.page_count, config.workspace.page_size, config.affinity, "0600"]);
        ...
        let regex = regex::Regex::new("^\\s+inet ([0-9]+\\.[0-9]+\\.[0-9]+\\.[0-9]+)\\/.*$").unwrap();
        let output = String::from_utf8(output.stdout).unwrap();
        let listen_addresses = output.lines().map(|x| regex.captures(x)).filter(|x| x.is_some()).map(|x| x.unwrap().get(1).unwrap().as_str().to_string()).collect::<Vec<String>>();
        if listen_addresses.is_empty() {
            panic!("Found no IP addresses on interface {}", config.tiles.quic.interface);
        }
        info!("[Configure] frank ... listening on {:?}", &listen_addresses);
        ...
        let src_mac_address = output.lines().find(|x| x.contains("link/ether")).unwrap().trim().split_whitespace().nth(1).unwrap();
        ...
        let status = Command::new("openssl")
            ...
            .args(["req", "-x509", "-newkey", "ed25519", "-days", "365", "-nodes", "-keyout",
                   "key.pem", "-out", "cert.pem", "-subj", "/CN=localhost", "-addext",
        ...
        config.frank = FrankConfig {
            ...
            main_cnc: main_cnc.split(":").collect::<Vec<&str>>()[1].parse().unwrap(),
    ...
    fn check(&mut self, config: &Config) -> CheckResult {
        ...
        for size in ["normal", "huge", "gigantic"] {
            ...
            match fs::metadata(&path) {
                Ok(_) => return CheckResult::Err(CheckError::PartiallyConfigured(format!("file {} exists", &path))),
                Err(err) if err.kind() == ErrorKind::NotFound => continue,
                result => return CheckResult::Err(CheckError::PartiallyConfigured(format!("error reading {} {result:?}", &path))),
            };
        }

        CheckResult::Err(CheckError::NotConfigured(format!("no files in {}", &config.shmem.path)))
    }
}
