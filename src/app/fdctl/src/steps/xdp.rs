...
impl Step for Xdp {
    ...
    fn check(&mut self, config: &Config) -> CheckResult {
        ...
        match fs::metadata(&xdp_path) {
            Ok(_) => (),
            Err(err) if err.kind() == ErrorKind::NotFound => return CheckResult::Err(CheckError::NotConfigured(format!("{xdp_path} does not exist"))),
            result => return CheckResult::Err(CheckError::PartiallyConfigured(format!("error reading path {xdp_path} {result:?}"))),
        };

        check_directory("/sys/fs/bpf", config.uid, 0, 0o40750)?;
        check_directory(&xdp_path, config.uid, 0, 0o40750)?;
        check_file(&format!("/sys/fs/bpf/{}/udp_dsts", config.name), config.uid, 0, 0o100640)?;
